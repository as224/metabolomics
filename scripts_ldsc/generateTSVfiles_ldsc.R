...
# Check if a row or column is completely zero -> if yes, delete
zero_rows <- rowSums(matrix_df == 0) == ncol(matrix_df)
zero_cols <- colSums(matrix_df == 0) == nrow(matrix_df)

# Keep rows and columns that are not all zeros
matrix_df <- matrix_df[!zero_rows, !zero_cols]

zero_rows_wo <- rowSums(matrix_df_woCorrection == 0) == ncol(matrix_df_woCorrection)
zero_cols_wo <- colSums(matrix_df_woCorrection == 0) == nrow(matrix_df_woCorrection)

# Keep rows and columns that are not all zeros
matrix_df_woCorrection <- matrix_df_woCorrection[!zero_rows_wo, !zero_cols_wo]
